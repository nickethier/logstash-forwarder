...
class LogStashForwarder::Event
  ...
  def initialize(data=Hash.new)
    ...
    if !@data.include?("@timestamp")
      @data["@timestamp"] = LogStashForwarder::Time.now.utc.to_iso8601
    end
  end # def initialize

  public
  def self.from_json(json)
    return LogStashForwarder::Event.new(JSON.parse(json))
