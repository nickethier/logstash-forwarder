...
require "logstash-lite/glob"
require "logstash-lite/event"
require "logstash-lite/logging"
require "logstash-lite/config/file"
require "optparse"



class LogStashLite::Agent
  attr_reader :config
  ...
  def initialize(args)
    ...
    @logger = LogStashLite::Logger.new(STDERR)
    @verbose = 0
    ...
    configure
    if @config_path
      config = LogStashLite::Config::File.new(nil, File.new(@config_path).read)
    else # @config_string
      # Given a config string by the user (via the '-e' flag)
      config = LogStashLite::Config::File.new(nil, @config_string)
    end
    ...
    EventMachine.run do
      ...
      @config["tail"].each do |glob|
        ...
        tail.subscribe do |path, data|
          event = LogStashLite::Event.new({"@host" => @nodename, "@source" => path, "@type" => tail.type, "@message"=>data})
  ...
  def options(opts)
    ...
    opts.on("-f CONFIGPATH", "--config CONFIGPATH", 
            "Load the logstash-lite config from a specific file.") do |arg|
      @config_path = arg
    end # -f / --config
