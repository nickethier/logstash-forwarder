...
require "logstash-lite/config/grammar"
require "logstash-lite/config/registry"
require "logstash-lite/agent"

class LogStashLite::Config::File
  public
  ...
  public
  def parse
    grammar = LogStashLite::Config::Grammar.new
    ...
    registry = LogStashLite::Config::Registry::registry
    each do |o|
  ...
  def tryload(parent, child)
    ...
    begin
      loaded = (require "logstash-lite/#{parent}s/#{child}")
      #if loaded
        #puts "Loading logstash-lite/#{parent}s/#{child}"
      #end
    rescue => e
      if child == :base
...
end #  class LogStashLite::Config::Parser
